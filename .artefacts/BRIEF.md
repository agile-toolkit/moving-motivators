...
- [x] JSON import of session history — "Import history" button (⬆️) in `SessionShiftPanel` (ResultsView.tsx) and `SessionHistoryPanel` (TeamSession.tsx) reads a `.json` file via hidden `<input type="file">` + `FileReader` API; validates schema (solo: `date`/`savedAt`/`ranked`; team: `sessionId`/`date`/`topMotivators`/`participantCount`); merges with existing history deduped by `savedAt`/`sessionId`, sorted newest-first, capped at 20/10; `results.importHistory` / `team.history.import` i18n keys in all 4 locales; zero new deps (issue #50)
...
- [x] [#50] Research: import session history from JSON (companion to #47 export) — implemented 2026-07-13
...
### 2026-07-13 — feat: JSON import of session history (issue #50)
- Done: auto-approved #50 (research companion to #47 export, `needs-review` since 2026-06-26, 17 days stale; comment posted on the issue). Added import button (⬆️ "Import history") in `SessionShiftPanel` (`ResultsView.tsx`) next to existing export button — hidden `<input type="file" accept=".json">` + `FileReader` reads the file, `isValidSessionEntry` guard validates shape (`date`/`savedAt`/`ranked`), merges incoming with existing `sessionHistory` deduped by `savedAt`, sorted newest-first, capped at 20, updates both state and `localStorage`. Added analogous `isValidTeamHistoryEntry` guard and import button (⬆️ "Import") in `SessionHistoryPanel` (`TeamSession.tsx`) — converted `history` from inline `JSON.parse` to `useState` so imported data renders immediately; merge by `sessionId`, cap at 10. Added `results.importHistory` / `team.history.import` i18n keys to EN/ES/BE/RU. Zero new dependencies. Bumped `package.json` to 0.1.4.
- Remaining: #51 (Team Identity → MM pre-populate team name, needs-review 17 days stale → auto-approve next); #52 (coaching tips panel, needs-review 14 days stale); #53 (Improvement Board deep link, needs-review 14 days stale); #5 (favicon, research-more, skip); #16 (Dashboard side, agile-toolkit.github.io); #22 (Change Planner side, change-planner)
- Next task: check issues for human feedback; auto-approve #51 (Team Identity → MM pre-populate team name) if still needs-review and implement: in `TeamSession.tsx` host lobby, read `team-identity:draft` from localStorage on mount; if non-empty `.name`, show suggestion chip "Use team name from Team Identity: _name_"; one-click sets `teamName` field in stored `teamSessionHistory` entry; `team.useTeamIdentityName` i18n key in all 4 locales; ~30 LOC
