 ...
 *    this app and Planning Poker (same org-level `VITE_FIREBASE_*` secrets),
 *    and both used to write `sessions/<pin>`. A PIN minted here could land on
 *    a live Planning Poker session, and vice versa — silently destroying it,
 *    since neither app checked.
 *
...
export function sessionPath(pin: string): string {
  return `sessions/moving-motivators/${pin}`
}
...
export async function claimSession(
  db: Database,
  build: (pin: string) => Record<string, unknown>,
  attempts = 5,
): Promise<string> {
  for (let i = 0; i < attempts; i++) {
    ...
    await set(ref(db, sessionPath(pin)), { ...build(pin), createdAt: serverTimestamp() })
    return pin
  }
