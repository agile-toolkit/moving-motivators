...
describe('sessionPath', () => {
  it('namespaces this app so it cannot collide with Planning Poker', () => {
    // Both apps share one Firebase project; both used to write sessions/<pin>.
    expect(sessionPath('123456')).toBe('sessions/moving-motivators/123456')
  })
})
...
describe('claimSession', () => {
  it('writes the session and returns its PIN', async () => {
    const pin = await claimSession(db, p => ({ pin: p, phase: 'lobby' }))
    expect(pin).toMatch(/^[0-9]{6}$/)
    expect(store.get(sessionPath(pin))).toMatchObject({ phase: 'lobby' })
  })

  it('hands the claimed PIN to the builder, so the stored pin field matches its path', async () => {
    // The security rules reject a session whose `pin` field disagrees with the
    // path it sits at, so the builder has to be told which PIN it won.
    const pin = await claimSession(db, p => ({ pin: p, phase: 'lobby' }))
    expect(store.get(sessionPath(pin))).toMatchObject({ pin })
  })
  ...
  it('reuses a PIN whose session has expired', async () => {
    ...
    expect(await claimSession(db, p => ({ pin: p, phase: 'lobby' }))).toBe(stale)
  })
  ...
  it('gives up rather than hammering a saturated namespace', async () => {
    ...
    await expect(claimSession(db, p => ({ pin: p, phase: 'lobby' }), 3)).rejects.toBeInstanceOf(
      SessionPinUnavailableError,
    )
  })
})
