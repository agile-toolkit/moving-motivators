### Moving Motivators — Goal

#### Outcome

Become the default digital Moving Motivators — the version practitioners
link to when they explain or run the exercise.

#### Why this app can win it

The physical card exercise breaks down the moment a team is distributed,
and the Management 3.0 community has no free digital version worth linking to.
A team mode that works over a shared link is the whole gap.

#### Signals

1. Inbound links from coaching blogs, trainings, and Management 3.0 material.
2. Ratio of team sessions to solo sessions — team use is the advocacy signal.
3. GitHub stars.
4. Sessions that end in a shared or exported result.

#### Boundaries

- Not a full Management 3.0 suite.
- Not a coaching marketplace or directory.
- The card set stays faithful to the original exercise.

*Repo: `agile-toolkit/moving-motivators`*
